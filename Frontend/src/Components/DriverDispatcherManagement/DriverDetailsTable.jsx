...
export default function DriverDetailsTable() {
  ...
  useEffect(() => {
    async function fetchDrivers() {
      try {
        const response = await axios.get('http://localhost:8000/driver-dispatcher');
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8000/driver-dispatcher/${id}`);
